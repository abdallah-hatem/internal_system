...
/**
 * The origin this server is reached on, as the client sees it.
 *
 * `PUBLIC_BASE_URL` when it is set — in production it should be, because the
 * forwarded headers are whatever the caller sent. Without it, the proxy's
 * `x-forwarded-proto` / `x-forwarded-host` and then the request's own host:
 * behind Vercel's proxy the request's protocol is plain http, and metadata
 * pointing Claude at http would send it nowhere.
 */
export function publicBaseUrl(
  ...
  const proto = first(headers['x-forwarded-proto']) ?? protocol;
  const host =
